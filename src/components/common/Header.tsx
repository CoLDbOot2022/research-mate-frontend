            ...
            <div className="flex items-center space-x-2 md:space-x-4">
              {me ? (
                <div className="relative" ref={menuRef}>
                  ...
                  {isMenuOpen && (
                    <div className="absolute right-0 top-12 w-64 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-xl animate-in fade-in slide-in-from-top-2 duration-200">
                      ...
                      <div className="py-1">
                        <button type="button" className="flex w-full items-center gap-3 px-4 py-3 text-left text-sm font-medium text-slate-700 hover:bg-slate-50" onClick={() => { setIsMenuOpen(false); router.push("/my-reports"); }}>
                          <LayoutDashboard className="w-4 h-4 text-slate-400" /> 기록 페이지
                        </button>
                        <button type="button" className="flex w-full items-center gap-3 px-4 py-3 text-left text-sm font-medium text-slate-700 hover:bg-slate-50" onClick={() => { setIsMenuOpen(false); router.push("/my-page"); }}>
                          <CircleUserRound className="w-4 h-4 text-slate-400" /> 마이페이지
                        </button>
                        <button type="button" className="flex w-full items-center gap-3 px-4 py-3 text-left text-sm font-medium text-slate-700 hover:bg-slate-50" onClick={() => { setIsMenuOpen(false); router.push("/credits"); }}>
                          <CreditCard className="w-4 h-4 text-slate-400" /> 이용권 충전
                        </button>
                        {me.email === "coldbootcp@gmail.com" && (
                          <button type="button" className="flex w-full items-center gap-3 px-4 py-3 text-left text-sm font-medium text-indigo-700 hover:bg-indigo-50" onClick={() => { setIsMenuOpen(false); router.push("/admin"); }}>
                            <Shield className="w-4 h-4" /> 관리자
                          </button>
                        )}
                      </div>
                      <button type="button" className="flex w-full items-center gap-3 border-t border-slate-100 px-4 py-3 text-left text-sm font-medium text-rose-600 hover:bg-rose-50" onClick={logout}>
                ...
                <div className="flex items-center gap-1 shrink-0">
                  <Button variant="ghost" size="sm" className="h-8 text-[11px] md:text-sm font-bold text-slate-600 hover:text-blue-600 rounded-full px-2" onClick={() => router.push("/login")}>로그인</Button>
              ...
              <button
                ...
                className="flex md:hidden p-2 text-slate-700 hover:bg-slate-100 active:bg-slate-200 rounded-full relative z-[110] items-center justify-center border border-slate-200 bg-white shadow-sm shrink-0"
                aria-label="메뉴 열기"
              >
